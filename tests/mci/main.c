...
unsigned char teststring1[SECTOR_SIZE];
unsigned char teststring2[SECTOR_SIZE];
...
void fill_test_strings(void) {
    ...
    teststring1[SECTOR_SIZE-1] = teststring2[SECTOR_SIZE-1] = '\0';
...
static void main_test(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, status);
    TEST_ASSERT_EQUAL_STRING((char*)teststring1, (char*)read_buffer);
    ...
    printf("Read the following:\n'%s'\n", read_buffer);
    TEST_ASSERT_EQUAL_INT(0, status);
    TEST_ASSERT_EQUAL_STRING((char*)teststring2, (char*)read_buffer);

    printf("About to erase sector 1\n");
    unsigned long block_to_erase = 1;
